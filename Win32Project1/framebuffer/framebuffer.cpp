...
FrameBuffer::FrameBuffer(float width,float height,bool depth) {
	this->width=width;
	this->height=height;
	hasDepth=depth;
	...
	colorBuffers.clear();
	colorBuffers.push_back(new Texture2D(width,height,TEXTURE_TYPE_COLOR));
	colorBufferCount=1;
	...
	depthBuffer=NULL;
	if(hasDepth) {
		depthBuffer=new Texture2D(width,height,TEXTURE_TYPE_DEPTH);
