...
Texture2D::Texture2D(float w,float h,int t,int p,int c) {
	...
	GLint depthType = precision == HIGH_PRE ? GL_FLOAT : GL_UNSIGNED_BYTE;
	...
	unsigned char* texData = (unsigned char*)malloc((width*height*c)*sizeof(unsigned char));
	memset(texData, 255, (width*height*c)*sizeof(unsigned char));

	switch(type) {
		case TEXTURE_TYPE_COLOR:
			glTexImage2D(GL_TEXTURE_2D, 0, preColor, width, height, 0, format, GL_UNSIGNED_BYTE, texData);
			break;
		case TEXTURE_TYPE_DEPTH:
			glTexImage2D(GL_TEXTURE_2D, 0, preDepth, width, height, 0, GL_DEPTH_COMPONENT, depthType, 0);
			break;
	}
